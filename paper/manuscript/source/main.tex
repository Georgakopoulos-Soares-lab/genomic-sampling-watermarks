...
in GENERator. The retained GENERator analysis reports the same 14 effects and their rescaled
confidence intervals, with the largest absolute standardized effect 0.111 standard deviations for
mean single-base-run length (Figure~\ref{fig:quality}b uses the Carbon panel; the corresponding
GENERator summary is retained in the analysis ledger and follows the same effect scale).
...
\caption{\textbf{Model likelihood and sequence-quality proxies.} The Carbon panel shows the
full 14-measure summary and the paired score difference. The corresponding GENERator summary is
retained in the analysis ledger and follows the same definitions and effect scale.
...
calibration. The retained GENERator analysis reports the same strength summaries and edit-specific
windows, with a weaker minimum margin than Carbon but no loss of detection in the tested reads.
...
\caption{\textbf{Position-independent detection.} Carbon gives the retained full window-strength
summary; the corresponding GENERator read-level and window summaries are retained in the analysis
ledger and follow the same detection rule and threshold correction.
...
The retained GENERator summaries include the same detection outcomes, window-strength separation,
and strongest-window-length counts as the Carbon analysis, computed from the existing version-one
trials without re-running generation or detection. All 384 marked reads were detected in each
condition, and its single wrong-key control positive was absent after deletion. Thus, both models
support detection after the tested single-base edits, while the evidence concerning changes in
window strength and length is reported for both models using the same definitions.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig4_edits}
\caption{\textbf{Effects of single-base edits on detection.} Carbon provides the retained full
window-strength and strongest-window summary; the same definitions are retained for GENERator from
its existing v1 trials. \textbf{a}, Strength of the best window in each of the 384 marked reads
per condition, on a log scale, with the median shown as a bar. The dashed line is the detection
threshold. Substitution produces a small reduction in median strength; insertion and deletion
reduce it by approximately half, while all marked reads remain above the threshold.
...
This work should be read as a scoped empirical evaluation rather than as a general proof of
watermark robustness, biological safety, or secret-key security. The study uses two models, one
watermark construction, one verifier, and one shared prompt cohort. Each model contributes a
single execution, and independent confirmatory replication remains outstanding. Both models use
six-base tokens, as do other genomic models \cite{wu2025generator}; models with single-base or
variable-length tokenisation would present different alignment requirements. Longer reads would
also increase the search correction. Because both models use the same prompts, the study evaluates
portability across models without independent replication across corpora.
...
each edit. GENERator provides the aligned-detector null diagnostic and the corresponding retained
window summaries from the same v1 trials. Claims based on these analyses are attributed to the
corresponding model; the empirical panels in the four figures use Carbon results, while the
corresponding GENERator summaries remain in the analysis ledger.
