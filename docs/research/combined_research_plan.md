...
The former Carbon-first plan is superseded. The active proposal is the
[dual-model SynthID paper rebuild plan](dual_model_synthid_paper_rebuild_plan.md).

The rebuilt paper will evaluate the current SynthID tournament sampler and position-independent
detector on both Carbon-500M and GENERator-v2 1.2B under one new study identity. Existing
