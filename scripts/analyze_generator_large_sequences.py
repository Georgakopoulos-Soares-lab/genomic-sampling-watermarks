#!/usr/bin/env python3
"""Compute GENERATOR likelihood and paired sequence proxies for the large corpus."""
...
def finalize(
    *,
    ...
    for metric in METRICS:
        main[metric] = compare_metric(
            ...
            seed=public_replay_seed(
                "generator-synthid-validation-v1/sequence-comparison",
                metric,
    ...
    for draw_id in sorted({int(row["draw_id"]) for row in rows}):
        per_draw[str(draw_id)] = {
            metric: compare_metric(
                ...
                permutation_replicates=args.permutation_replicates,
                seed=public_replay_seed(
                    "generator-synthid-validation-v1/sequence-comparison/per-draw",
                    str(draw_id),
                    metric,
                    str(args.analysis_seed),
                ),
    ...
    summary = {
        ...
        "question": "matched full-sequence GENERATOR behavior",
        "policy_id": "G_tok",
        ...
        "likelihood": {
            "definition": (
                ...
                "under the same frozen G_tok law, conditioned on the original prompt"
            ),
