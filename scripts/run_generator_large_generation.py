#!/usr/bin/env python3
"""Resumable matched SynthID tournament generation for one GENERATOR draw.
...
def next_session_path(sessions_dir: Path, draw_index: int) -> Path:
    ...
    return sessions_dir / f"draw_{draw_index:02d}_session_{time.time_ns()}_{os.getpid()}.json"
...
def main() -> int:
    ...
    if cases_pending:
        ...
        stream_domain = f"{args.experiment_label}/{cohort_id}/G_tok"
