#!/usr/bin/env python3
"""Strict end-to-end validator for GENERATOR SynthID validation artifacts."""
...
def main() -> int:
    ...
    require(generation["policy_id"] == "G_tok", "generation summary has the wrong policy")
    require(
        generation["model_id"] == "GenerTeam/GENERator-v2-eukaryote-1.2b-base",
        "generation summary has the wrong model",
    )
    require(
        generation["revision"] == "c41b0018da9ee13b9e96ee54647de8da381ccd72",
        "generation summary has the wrong model revision",
    )
    ...
    require(
        all(str(row["policy_id"]) == "G_tok" for row in records),
        "generated corpus contains the wrong model policy",
    )
    require(
        all(
    ...
    require(
        distribution_summary["experiment_id"] == "generator_synthid_validation_v1",
    ...
    require(
        ...
        "GENERATOR SynthID validation must use the frozen analytic standard-normal threshold",
    )
