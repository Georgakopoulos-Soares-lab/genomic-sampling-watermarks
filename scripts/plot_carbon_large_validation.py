#!/usr/bin/env python3
"""Generate ten Carbon SynthID validation diagnostic figures."""
...
def main() -> int:
    ...
    ax.set(
        xlabel="Ordinary empirical TV to C_tok",
    ...
    axes[0].set(xlabel="C_tok entropy (bits)", ylabel="Watermarked TV distance")
    axes[1].set(xlabel="C_tok top-1 mass", ylabel="Watermarked TV distance")
    ...
    axes[0].set(xlabel="C_tok entropy (bits)", ylabel="Fixed-key tournament TV to C_tok")
    axes[1].set(
        xlabel="C_tok top-1 mass",
        ylabel=f"{key_average_replicates}-key average TV to C_tok",
    )
    ...
    ax.set(
        ...
        title="Matched Carbon self-likelihood",
    )
